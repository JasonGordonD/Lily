...
class LilyGame:
    ...
    def _picture_kind_for_slot(self, rnd: int) -> str | None:
        ...
        entirely. The final wager round stays text. In pictures mode,
        round REAL_OR_IMAGINED_ROUND is the reference picture round
        (every question); otherwise the first question of each round is
        a real-entity 'name this landmark' slot."""
        if self.sk.media_mode != "pictures":
            return None
        if self.sk.mode == "adult":
            # Structured picture slots stay text in adult mode — the ROI
            # builder serves REAL web photos on even slots and the
            # real-photo rail never serves the adult deck. Adult imagery
            # runs through the GENERATED rail only (demo + generated
            # images), styled by lily_imagegen.lily_adult_style.
            return None
        if rnd > self.rounds_total:
