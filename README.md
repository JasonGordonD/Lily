...
| `min_endpointing_delay` (session, not STT) | 0.5 (framework default) | **0.6** (HOTFIX-005 X9) | the runtime's own remedy for the split-utterance class ("consider raising min_delay in the endpointing options to accommodate a slow stt"): the 0.5 floor commits the turn before the enhanced point's 1.5s `max_delay` delivers the final transcript. `LILY_STT_MIN_ENDPOINTING_DELAY`; ceiling (`max_endpointing_delay`) is **2.5** since WO-LILY-COMPOSITION-FOLLOWUP-001 L1 (was pinned at 6.0; the earlier "6.0 framework default" claim was wrong — livekit-agents 1.6.10 voice/turn.py ships max_delay 3.0, 2.5 with a streaming turn detector). Set WITH `max_delay`, not against it |
