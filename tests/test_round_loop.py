...
from lily_evaluation import (
    QUESTION_SPOKEN_PARAPHRASE_RATIO,
    lily_question_spoken_ratio,
)
from lily_scorekeeper import LilyScorekeeper


def _question_was_spoken(question_prompt: str, spoken_text: str) -> bool:
    """The agent-side decision at its loosest tier: the window opens when
    the spoken ratio clears the paraphrase threshold (lily_agent's
    on_agent_speech_finished; the min-2-matched-tokens guard lives inside
    lily_question_spoken_ratio). The gate logic is pure and lives in
    lily_evaluation so these tests stay out of the livekit import graph."""
    return (
        lily_question_spoken_ratio(question_prompt, spoken_text)
        >= QUESTION_SPOKEN_PARAPHRASE_RATIO
    )


# ---------------------------------------------------------------------------
# question-spoken gate — loosened, tiered
# ---------------------------------------------------------------------------
