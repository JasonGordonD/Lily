...
def test_voice_id_outcome_stamps_exist_in_the_match_path():
    ...
    # And both session-end metadata writes carry the block (WO-LILY-VOICE-
    # TRUTH-001 rule (f): ONE receipt builder at both sites).
    import lily_identity
    module_src = inspect.getsource(lily_agent)
    assert module_src.count('"voice_identity": game.voice_identity_receipt()') >= 2
    assert '"never_ran"' in inspect.getsource(lily_identity)
