...
| 80 | W3 negation-symmetric mode extraction + pacing confirmation beat / `LILY_PREFS \| PACING`, `pacing_confirm`/`pacing_kept` | `_PACING_TIMER_REFUSAL_RE` SK:265ff (checked first in `lily_detect_pacing_choice` SK:308), `_MEDIA_REFUSAL_RE` SK:987ff (checked first in `lily_detect_media_choice`); confirmation gate + resolver A:7423ff / A:7365ff, `_pending_pacing` | (a) A refused timer/clock/countdown/timed — or picture/image/screen — resolves to the OFF direction and wins the collision, so a protest ("I'm not playing with the timer") can never invert into enabling it. (b) A pacing change that contradicts `self.prefs["pacing"]` stated this session is held for ONE confirmation beat and applies only on the requester's next assent | WO-LILY-HOTFIX-009 W3 | **DELETED** `_PACING_TIMED_NEGATION_RE` (adjective-only, coverage narrower than the enable patterns) — subsumed by `_PACING_TIMER_REFUSAL_RE`, not a second guard. The confirmation beat reuses the forget pending-confirm pattern (mech. adjacent, `lily_parse_forget_confirmation`) and the existing per-group `prefs` store — no new store. Fixes the live inversion "Timed clock's back on — noted." after the table had chosen relaxed |
