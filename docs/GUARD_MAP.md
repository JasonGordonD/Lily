...
| 80 | W7 name-correction projection / `LILY_BIND \| NAME_EVIDENCE` -> `LILY_STATE \| PLAYER_RENAMED` | spelled-name inlet `lily_extract_spelled_name` (lily_binding.py, tried first inside `lily_extract_explicit_name`); evidence-driven re-bind in `note_confirmed_name_evidence`; migrate-not-fork `bind_speaker(rename=True)` (lily_scorekeeper.py) | A bound label's own voice states a corrected name (incl. NATO/letter-by-letter spelling) | The correction rides the EXISTING evidence -> bind -> `on_speaker_bound` -> attribute-publish path: player entry and ledger history migrate to the corrected spelling, no ghost entry, the glass updates in one seam update without waiting on a model tool re-call | HOTFIX-009 W7 |
