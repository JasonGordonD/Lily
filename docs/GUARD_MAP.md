...
|---|---|---|---|---|---|---|
| 84 | `correct_verdict` grounds/verdict/corroboration gate / `VERDICT_CORRECTION_NO_VERDICT`, `VERDICT_CORRECTION_BAD_GROUNDS`, `VERDICT_CORRECTION_NOT_DENIED`, `VERDICT_CORRECTION_NON_POSITIVE_DELTA`, `VERDICT_CORRECTION_UNCORROBORATED` | SK `correct_verdict` | A verdict-correction request | Refuses unless a prior `"answer"` row exists, grounds ∈ `CORRECTION_GROUNDS`, delta > 0, the verdict was a denial, **and** (for grounds=`answer_denied`) the recorded attempt fuzzy-matches canonical via the existing Tier-1 matcher (`lily_evaluation.lily_tier1_evaluate`). **This is the correction/fabrication boundary** — a correction can only amend a ruling that happened, and `answer_denied` (the highest-risk ground) is mechanically corroborated, not LLM-asserted, so a rightly-denied WRONG answer cannot be restored | HOTFIX-009 W1 | Reads `ledger_row_for` (WS-7-adjacent) + `lily_evaluation` (existing matcher, no new machinery); the ONLY writer of `verdict_correction`; feeds mech. 65 (standings sum it). Other grounds (wrong_rule/misheard/out_of_window) stay caller-judged within existing bounds. The `answer_denied` corroborating attempt comes from mech. 87 (in-session first, addressee_log fallback) |
| 87 | `answer_denied` corroboration source (in-session + addressee_log fallback) / `VERDICT_CORRECTION_DB_UNREACHABLE` | A `lily_correct_verdict` (leg 2 = SD-adjacent `lily_persistence.lily_fetch_inwindow_fuzzy_transcripts`) | grounds=`answer_denied` | Feeds mech. 84 a corroborating attempt: (1) the denied row's own transcript, else (2) a scoped session read of `lily_addressee_log` (`answer_window_open` + `fuzzy_matched_answer`) — the ONLY durable record of an answer IGNORED at scoring (rejected by one-candidate-per-player, never a ledger row; the diamond class). Each candidate is re-checked against canonical by mech. 84's matcher (the table has no `question_id`, so the match IS the question association). DB read failure ⇒ honest refuse ("evidence unavailable"), never a restore | HOTFIX-009 W1 (Option B) | Uses the supabase client the agent already holds; a single filtered SELECT, not new infrastructure. Gated to `answer_denied` only |
| 85 | Correction idempotency belt / `VERDICT_CORRECTION_ALREADY_CORRECTED` | SK `existing_correction` | A second correction of the same `(player, question_id)` | Refuses | HOTFIX-009 W1 | The correction analogue of the WS-4 award belt (`_existing_award`). Distinct from it: the WS-4 belt guards the `"answer"` cause and is **untouched**; this one guards `"verdict_correction"` |
| 86 | WS-4 belt exemption scoping | SK `apply_score_event` | `cause == "verdict_correction"` | The WS-4 idempotency belt is skipped for this cause ONLY (a correction is an intentional second movement) | HOTFIX-009 W1 | Load-bearing that this is scoped by cause AND that the cause has one caller (mech. 84, which requires a real prior verdict): the exemption cannot reopen the fabrication hole the WS-4 belt closes |

Tool surface: `lily_correct_verdict` (`A`) exposes mech. 84 to the LLM; the HOTFIX-005 X12 verdict-contest
directive (`_contest_note`, `on_transcript_event`) now names it. No mechanism was **removed** — the
anti-fabrication belt guards a different cause and stays whole. **New mechanism count: 87**
(mechs. 84–86 above + mech. 87, the `answer_denied` corroboration source added in the W1 Option B fix-loop;
renumbered from 81–84 on rebase — W6/W7/W3 landed mechs. 81–83 on main first).

**Amendment 2026-08-10 (WO-LILY-HOTFIX-009 W8 — the hold binds the delivery lane):** two mechanisms,
both on the hold-integrity axis. Mech. **88** (hold-narration integrity, §4) backs a spoken
"stopped/held" claim with actual hold state so the conversational brain can no longer narrate a hold
that mechanically never existed; mech. **89** (plain-hold prompt-state directive, §4) forbids the
model from producing any game payload — in prose or as the question — while a plain hold is set,
closing the prose-payload gap the mechanical delivery gate (armed-question-only) leaves open. Mech. 88
was authored as "81" against W8's isolated base tree (HOTFIX-008 tip + W2 only); it is renumbered to
**88** here because W6 landed mech. 81 on main first. No mechanism was **removed**. **New mechanism
count: 89.**

**Amendment 2026-09-06 (WO-LILY-SUPPLY-001 S1 — the bank serves, the author replenishes):** one
mechanism RETIRED, one added, two amended. This is a supply-ordering change, not a new guard layer:
the machinery all existed, and what moved is who is asked first.

| # | Name / log tag | file:line | Trigger | Action | WO | Interactions |
|---|---|---|---|---|---|---|
| 90 | Bank-first supply draw / `LILY_SUPPLY \| BANK_DRAW`, `BANK_DRY`, `AUTHOR_ON_DELIVERY_PATH`, `MC_DEGRADED` | `_bank_draw` lily_supply.py:107–184, `_author_draw` :186–216, `_deck_for_supply` :90–105, receipt `supply_receipt` :60–88 | Every text-supply draw: prefetch (`_prefetch_inner`), the Z2 recovery ladder and the WS-6 fallback (`_bank_to_supply`), and the CLASS 6 topic-released redraw | Draws `lily_questions` filtered by **deck** (`adult` column, from `availability_flags["adult_deck"]`), **lane** (the rotation family mapped onto the bank's own category vocabulary, `lily_bank.lily_lane_categories`) and **register** (`difficulty_tier`, relaxed across the whole lane before the lane is left), excluding `_no_repeat_exclusion()` (group asked-history + session drawn set + WS-4 burned) and the group's played answers. The author is reached ONLY on a dry lane and every such call is counted into `session_metrics.supply.generation_calls_on_delivery_path` (expected 0) | **WO-LILY-SUPPLY-001 S1** | THE single door into `lily_fetch_bank_question` — mech. 32 (WS-6 fallback) and mech. 79 (Z2 ladder) now reach the bank through it, which is how `_bank_to_supply`'s hand-rolled exclusion union (history \| drawn, **burned omitted**) stopped diverging from every other draw path. Feeds mech. 66's burn set back into the draw it was always meant to gate. Does not touch delivery: the row lands on `next_question` and the phase machinery still owns arming (mech. 79's contract, unchanged) |

**RETIRED: the insurance-bank leg** (`LILY_PREFETCH | INSURANCE_BANK_HIT` / `INSURANCE_BANK_EMPTY` /
`INSURANCE_BANK_ERROR`, HOTFIX-008 Z2's telemetry on it, and its awaited `ensure_choices` MC
synthesis). It was mech. 79's "prefetch insurance leg". It ran the SAME `lily_fetch_bank_question`
with the SAME arguments as the draw that now runs first, so after the inversion it could only ever
repeat a miss. Its purpose — "generation failed, cover the draw" — IS the new ordering. Deleted, not
left dead. `test_supply_recovery_z2.py`'s two insurance-telemetry tests are re-pointed at
`BANK_DRAW` / `BANK_DRY`; Z2's actual requirement (this draw never runs untelemetered again) is
carried, not dropped.

**AMENDED, mech. 79 (Z2 phase-independent supply recovery):** its `INSURANCE_BANK_*` markers are gone
per the retirement above; `_bank_to_supply` and the recovery ladder are otherwise unmodified and still
land rows on the supply line only. Its de-escalated re-prefetch rung now runs AFTER the bank has
already been asked, so on a stocked bank the ladder rarely reaches an author at all.
...
| # | Name / log tag | file:line | Trigger | Action | WO | Interactions |
|---|---|---|---|---|---|---|
| 5 | Hold gate / `LILY_SAY_SUPPRESSED \| reason=hold` | `hold_blocks_dispatch` A:4487–4495; call SD:119–124 | `_hold_active` and source not in `_HOLD_EXEMPT_SOURCES` (`stop_primitive`, `hold_ack`, `hold_release`, A:4443–4446) | Refuses the dispatch | PATCH-002 A4 | Binds **every `gated_say` lane** including the idle watchdog (mech. 15 checks it at A:3882–3886). Entered by mech. 6b (self wait-promise), the STOP primitive (mech. 33), a decline; released by any user final, a hard game event, or timeout. **The one delivery path outside `gated_say` — the organic/structural `register_delivery_claim` lane (mech. 13) — is bound to this same `_hold_active` state directly since WO-LILY-HOTFIX-009 W2** |
...
| 13 | `register_delivery_claim` — THE delivery event | SD:638–780; call A:12814–12832 | Every outbound turn while a question is armed and the window is closed | Returns `claimed_structural` / `claimed_core_sentence` / `rewrite_strict` / `duplicate` / `held` / `None`; claims `q_{N}_delivery` | desync WO Sub-agent B, WS-1; **hold check WO-LILY-HOTFIX-009 W2** | Text similarity is **telemetry only** (mech. 43); the claim is the sole delivery truth. Feeds mech. 26 (window open), mech. 30 (reconcile), mech. 47 (MC start). **W2: this lane now consults the hold (mech. 5's state).** It is the one delivery path that does NOT pass `gated_say`, so before W2 the hold bound every code-dispatch lane but not an organic/structural turn performing the armed question — live `lily-5E3036` q_6 aired at 05:40:11 as a `source=tts_node trigger=structural` claim. Now: `_hold_active` and the turn textually performs the armed question → returns `held`, made physically silent by the caller (A:13595, same seam as `duplicate`). Keyed on the question being in the turn's text so the owed conversational reply still speaks; STOP/sticky is stronger and returns `None` above, unchanged |
...
| 33 | STOP primitive / `LILY_STOP \| PRIMITIVE`, `RESUMED` | `handle_stop_primitive` A:4680–4737; `_freeze_game_delivery_for_stop` A:4616; consult `maybe_route_stop` | STOP utterance | Retires armed/prefetched content, kills the watchdog's ability to resurrect the turn, enters the hold, one brief ack | PATCH-002 A5/T12, P0-B; **consult + emphatic-repetition routing WO-LILY-HOTFIX-009 W8** | The consult (roster read → `lily_detect_stop` → route) is at the very top of the user-final path so it bypasses every other gate. **W8:** the consult is extracted into `maybe_route_stop` (offline-testable on the production path) and `lily_detect_stop` now fires the **emphatic-repetition register** ("stop stop stop", ≥3 stop-words) regardless of roster. The solo gate only disambiguates who a *lone* stop addresses; a shouted repetition removes that ambiguity. Live `lily-5E3036` 05:39:10 "Stop stop stop stop stop stop stop" DETECTED (True with solo) but did NOT route because a phantom second player ("Rummy", W7's territory) made `roster_size()` read 2 (solo=False) and a bare stop counts only in solo — zero `LILY_STOP`/`LILY_HOLD` in the whole trace. Now roster-independent for the emphatic register; addressed/quit-game/solo-bare stops all unchanged |
...
| 88 | Hold-narration integrity / `LILY_HOLD \| NARRATION_BACKED` | `back_hold_narration`; detector `lily_detect_hold_narration` SK; call in `tts_node` just before mech. 13 | An outbound turn narrates a stopped/hold state (a "stopped" assertion + a hold cue: `still stopped` / "I'm listening" / a "say the word / until you say go" release invitation) while `_hold_active` is False **and** the game is not under the sticky STOP latch | Enters the hold (`enter_hold`, reason `narrated_stop`) so the spoken claim is backed by state — a correction, not a suppression | **WO-LILY-HOTFIX-009 W8** | The structural-claim honesty organ (mech. 13) prevents *delivery* claims without state; this is its analogue for the *hold* claim. Sets `_hold_active` only; the in-flight ack has already passed `gated_say`, so backing the claim never silences the ack — it binds the NEXT dispatch, which with mech. 5+W2 is the leaked delivery. Live `lily-5E3036`: three "Stopped…/Still stopped…/Stopped until you say go" turns (05:39:26–05:40:00) narrated a hold that mechanically never existed (no `LILY_STOP` upstream, mech. 33), and q_6 aired at 05:40:11 with no hold to block it. No-op when already held (the legitimate mech. 33 ack path) or under sticky STOP (the claim is backed by mech. 10). **W8 review Finding 3:** the `still` cue is bound to the `still stopped` adjacency so recap/scorekeeping patter ("still tied at two") cannot enter a hold and suppress a same-turn delivery — the brake may over-fire recoverably but never under a legitimate payload. Numbered 81 against W8's isolated base tree; renumbered 88 at landing (W6 owns 81 on main) |
| 89 | Plain-hold prompt-state directive / `[GAME STATE] held` | `_state_extra_lines` A:11114 (the `elif _hold_active` branch, mutually exclusive with the sticky-STOP `game_delivery: STOPPED` directive above it) | `_hold_active` True while the sticky STOP latch is NOT set (a self-wait-promise, a decline, P10, or a mech. 88 backed stopped-state narration) | Emits a context-only `held:` directive forbidding the model from producing ANY game payload — ask/arm/reveal/score/nudge/steal — in prose OR as the question, while held; acknowledge only that you've stopped and are listening | **WO-LILY-HOTFIX-009 W8 residual** | The delivery lane mechanically suppresses only the ARMED-QUESTION form (mech. 13 + W2); an organic turn narrating a reveal/verdict/steal/score in PROSE would slip that gate and air under the hold (W2 review Residual i). Closes it at the prompt surface rather than broadening the delivery matcher into a content classifier (which risks locked-canon false-positive suppression). Mutually exclusive with the sticky-STOP directive (no double-directive); the mechanical delivery gate stays the backstop for the armed-question form. Same context-only, leak-filtered contract as the STOPPED directive |
...
| 83 | W3 negation-symmetric mode extraction + pacing confirmation beat / `LILY_PREFS \| PACING`, `pacing_confirm`/`pacing_kept` | SHARED `_build_refusal_re(noun)` SK feeds `_PACING_TIMER_REFUSAL_RE` (first in `lily_detect_pacing_choice`), `_MEDIA_REFUSAL_RE` (first in `lily_detect_media_choice`), `_ADULT_DECK_REFUSAL_RE` (gates `lily_parse_lobby_setup_intents["adult"]`); confirmation gate + resolver A:7423ff / A:7365ff, `_pending_pacing`, provenance `_pacing_stated_this_session` | (a) A refused mode noun (timer/clock/countdown/timed · picture/image/screen · adult deck) resolves to OFF and wins the collision across ALL THREE deterministic extraction surfaces, so a protest ("I'm not playing with the timer", "don't turn the timer on", "I don't want the adult deck") can never invert into enabling it. (b) A pacing change contradicting `self.prefs["pacing"]` is held for ONE confirmation beat, applied only on assent, worded by real provenance (stated-this-session vs remembered-from-memory) | WO-LILY-HOTFIX-009 W3 (+fix-loop) | **DELETED** `_PACING_TIMED_NEGATION_RE` (adjective-only, narrower than the enable patterns) — subsumed by the shared builder. All negation routes through ONE builder so a new surface inherits it (consolidation mandate). Beat reuses the forget pending-confirm pattern (`lily_parse_forget_confirmation`) and the existing per-group `prefs` store — no new store. Fixes the live inversion "Timed clock's back on — noted." after the table chose relaxed |
...
| 82 | W7 name-correction projection / `LILY_BIND \| NAME_EVIDENCE` -> `LILY_STATE \| PLAYER_RENAMED` (or `RENAME_REFUSED`) | spelled-name inlet `lily_extract_spelled_name` (lily_binding.py, tried first inside `lily_extract_explicit_name`; canonical-NATO-majority rule so informal-word runs stay stated names); evidence-driven re-bind in `note_confirmed_name_evidence`; migrate-not-fork `bind_speaker(rename=True)` (lily_scorekeeper.py) gated by `lily_names_probably_same` at the migration writer | A bound label's own voice states a corrected name (incl. NATO/letter-by-letter spelling) that plausibly respells the bound one | The correction rides the EXISTING evidence -> bind -> `on_speaker_bound` -> attribute-publish path: player entry and ledger history migrate to the corrected spelling, no ghost entry, the glass updates in one seam update without waiting on a model tool re-call; a DISSIMILAR name (second voice on a reused label) is refused migration and stays a new-binding decision | HOTFIX-009 W7 |
...
| HOTFIX-009 W8 (5E3036 hold not binding the delivery lane) | 88 (analogue of 13 for the hold claim), 89 (prompt-surface backstop to 13+W2) |
...
## Addendum — WO-LILY-OPERATOR-MODS-001 (2026-09-06): three gates added, none of the above removed

| # | Mechanism / log line | Where | Condition | Effect | Origin | Interactions |
|---|---|---|---|---|---|---|
| 60 | B7 reply-owed latch / `LILY_REPLY \| OWED_AIRED` · `OWED_ENDED` · `OWED_CODE_OWNED` · `OWED_TIMEOUT` · `OWED_RELEASED_DISPATCH`; `LILY_PROGRESSION \| PAUSED … reason=reply_owed` | `lily_speech_delivery.note_user_turn` → `_begin_reply_owed`; `reply_owed_reason`; `note_playout_started` → `_note_reply_owed_airing`; `lily_floor.note_dispatch_playout` → `note_reply_owed_speech_end`; read by `lily_floor.progression_paused_reason` (mech. 14 gains a seventh state) | A user turn committed (`on_user_turn_completed`) and the organic reply handle the framework created for it (the post-commit handle no Lily lane stamped) has not reached its first frame; grace 2.0 s while no handle exists yet; cap 20 s | `dispatch_armed_question` refuses with `reply_owed`; the C3d barge-resume watch and `_maybe_resume_mcq_read` defer; the held question is dispatched at the reply's END (`call_soon`, not the 10 s watchdog) | operator B7 (live 11:50:38Z: the next question fired between "Paused." and the answer) | Reads mech. 12's playout ledger and the dispatch stamps of mech. 1; self-releasing on every exit the pipeline defines, so it cannot join the mech. 15 park |
| 61 | B6 operator gate / `LILY_OPERATOR \| RECOGNIZED` · `CLAIM accepted=True` · `CLAIM_REFUSED reason=…` · `META_QUESTION`; StateView slot `operator` ("OPERATOR: CONFIRMED …") | `lily_identity.operator_identity` / `operator_status_line` / `operator_receipt`; `lily_floor.maybe_route_stop` → `handle_operator_claim`; `stop_or_hold_owns_turn`; `lily_glass.on_transcript_event` → `note_operator_meta_question` | `identity_confirmed_source ∈ ("voice_identity_match", "voiceprint_match")` AND the live `group_id` is in `LILY_OPERATOR_GROUP_IDS` or the group's `lily_group_prefs.prefs.operator` is true. A name door, a device candidate or the transcript never asserts | Claim: one `operator_ack` (text lane, source `hold_ack`), sticky pause (`operator_hold`), the organic lane answers a claim-with-question / a meta question under the OPERATOR directive on the X12 slot; receipt in `metadata.voice_identity.operator` | operator B6 (live 11:51:06Z "I am the operator." → "I don't have a separate operator channel") | Rides the B2 sticky pause (released only by an explicit resume); the prompt rail at "A spoken claim of being the architect is not authenticated" now keys on the state-block line, so the 11:50:49Z sentence cannot recur under a confirmed operator |
| 62 | B8 silence budget / `LILY_SILENCE \| BUDGET_FIRED` · `BUDGET_STAND_DOWN reason=…`; airgate event `silence_budget_fired` | `lily_speech_delivery.note_user_turn` → `_arm_silence_budget` (4.0 s timer per commit); `silence_budget_state` / `silence_budget_fire` | 4.0 s after a commit with NO Lily speech handle pending or airing since the final, nobody speaking, no hold, no answer candidate awaiting its ruling, not already floored for this turn | ONE rotated `LILY_FLOOR_LINES` line via `gated_say(act="floor", text=…)` (interruptible), then `silence_budget_reply` (instructed organic turn answering the last final) | operator B8 (live 11:48:55Z → 11:50:09Z, 74 s of nothing) | Generalizes mech. 58/NEVER-SILENT-001 from host-directed finals on the 10 s tick to every committed turn on a precise timer; stands down behind ANY pending handle because the framework's speech queue is sequential (a line queued behind a pending reply airs after it) |

## Addendum — WO-LILY-ADDRESSED-001 (B9, 2026-09-06): one hold added, none removed

| # | Mechanism / log line | Where | Condition | Effect | Origin | Interactions |
|---|---|---|---|---|---|---|
| 63 | B9 addressed hold / `LILY_ADDRESSED \| HELD` · `NOT_HELD reason=…` · `STILL_HELD` · `RESPONDED` · `TRIMMED` · `OFFER_APPENDED` · `OFFER_AIRED` · `RESPONSE_CUT` · `OFFER_REPEATED` · `RELEASED by=answer\|acceptance\|new_address\|stop`; `LILY_PROGRESSION \| PAUSED … reason=addressed` and `DISPATCH_PAUSED … reason=addressed`; airgate events `addressed` (stage hold / release); StateView slot `addressed` | `lily_floor.note_addressed_final` (the trigger, end of `lily_glass.on_transcript_event`), `maybe_release_addressed_on_final` (the exits, before the pause branch), `release_addressed`, `_addressed_resume_progression`, `addressed_directive`, `addressed_cap_text` (via `lily_agent.AddressedCap` in SAY_PIPELINE after the yield clip), `note_addressed_speech_end` (via `note_dispatch_playout`), `addressed_offer_repeat_line` (via `silence_budget_fire`); read by `progression_paused_reason` (mech. 14 gains an eighth state), gated_say's `_PROGRESSION_ACTS` chokepoint, the C3c/C3d resume paths, `question_pending_blocks_dispatch` (silence_budget exemption) | FL-1 `host_directed` (score ≥ 0.60 or a hard rule) AND the final did not become an answer candidate AND a live game AND no code lane routed it (command / media / code-acked / reply to her question / backchannel / bare affirmative / acceptance / pause request) | Progression holds (no question lane dispatches, resume watch defers, watchdog halts, open-window clock held as B2 holds it); the first organic turn is capped to its contract (3 body sentences for question/structural, 2 otherwise, none for game_meta) and closed with the operator's offer; released ONLY by the table — an answer in the open window, an acceptance (then the held read / armed question dispatches as the reply), a new address (cycle restarts), STOP | operator's B9 scope correction (live lily-C47CD4 14:26:28Z "Who's Alfred Kingsley, anyways?" answered fused with "Next one." 24 s later; the month's class: name spelling, "I hate that word", "why are you so slow", "I said diamond", "we're not talking to you") | Precedence: STOP > pause/hold > addressed > reply_owed; the offer sets P6's `_question_pending` (an acceptance releases it ahead of the dispatch; a non-acceptance after the offer is NOT exempt as "answered her question", so the cycle can restart); rides P9's address debt untouched (a response that never reached playout still refuses progression on `address_unanswered`); mech. 62's line becomes the offer once per hold |

## Addendum — WO-LILY-SUPPLY-001 S2 (2026-09-06): two background gates added, none removed

Operator ruling: *"the bank serves, the author replenishes. Live question authoring never sits on the
delivery path again."* Both mechanisms below live entirely in the BACKGROUND lane
(`lily_bank_replenish.py`) and neither can refuse, delay, rewrite or suppress a spoken turn — that is
the point of them. They are recorded here because they gate what reaches the bank the delivery path
draws from, which is the one way a background job can hurt a table: by putting a duplicate, an
unverified, or a moderation-refused question where the draw will find it.

| # | Mechanism / log line | Where | Condition | Effect | Origin | Interactions |
|---|---|---|---|---|---|---|
| 64 | S2 bank watermark / `LILY_BANK \| WATERMARK \| lane= ready= target= consumed_pct=` · `REPLENISH_START` · `REPLENISH_DONE` · `REPLENISH_FAILED` · `REPLENISH_SKIPPED reason=already_running` · `REPLENISH_BLOCKED` · `REPLENISH \| … cost_tokens=` | `lily_bank_replenish.lily_bank_watermark` / `lily_bank_should_replenish` / `lily_replenish_sweep` / `lily_bank_replenish_loop`; started by ONE guarded `lily_spawn` in `lily_agent.entrypoint` (in the boot fan-out beside the embedder prewarm and the report sweep), stopped by `lily_stop_background_author` on `ctx.add_shutdown_callback` | A lane (`<deck>:<category>` of the rotation) is 40% CONSUMED — `lily_arsenal.lily_replenish_threshold(target, ratio)` reused verbatim, evaluated PER LANE independently — measured either as shortfall against `lily_config.bank_target_depth(lane)` or as serves this session | Opens ONE replenishment run for THAT LANE (`lily_bank_replenish_runs`, partial unique index on lane WHERE status='running' — a second run stands down), authors toward target, closes the receipt with its token cost. Never awaited by the game; every failure swallowed and counted; CancelledError re-raised unchanged | WO-LILY-SUPPLY-001 S2; the picture arsenal's own watermark (mech. — `lily_arsenal.lily_should_replenish`, ARSENAL-SEED A8) applied to text | Reads nothing the delivery path writes and holds no lock it wants. Independent of mechs. 32/60/61/79 (the SUPPLY recovery ladder): those recover a dead supply LINE inside a session, this stocks the BANK those rungs draw from. `LILY_BANK_REPLENISH_ENABLED` ships OFF; `scripts/bank_replenish.py` runs the same sweep out of session |
| 65 | S2 bank-write gate / `LILY_BANK \| DUP_REJECTED` · `MODERATION_REJECTED` · `VERIFY_REJECTED` · `BANKED` · `INSERT_FAILED` | `lily_bank_replenish.lily_replenish_lane` (the per-slot pipeline), `lily_bank_find_duplicate`, `lily_bank_text_sha256`, `lily_ready_row`, `lily_bank_insert_ready`; migration 029's `lily_questions_sha256_unique_idx` | Every authored candidate, before it can be banked: (1) the EXISTING verify step (`lily_reasoning.verify_question`, 3-7 s) must pass; (2) exact sha256 of the normalized text must not already exist and the ARSENAL-SEED A5 similarity pass (`lily_bank.lily_find_duplicate`, difflib ≥ 0.87 in-category) must not match a lane row; (3) a provider content refusal is classified by `lily_arsenal_gen.lily_is_moderation_rejection` | The candidate is DROPPED and COUNTED (per-outcome counter on the run receipt + a per-outcome log line); only a survivor is INSERTed, as `status='ready'` with lane, `question_text_sha256`, `replenished_at` and `replenish_run_id`. A read failure in the dup check FAILS CLOSED (the candidate is skipped) | WO-LILY-SUPPLY-001 S2; ARSENAL-SEED A5/A9 | The unique index is the last line of defence when two runs race on identical text (one insert, one honest failure). `status='ready'` is the seam with S1's draw, which accepts 'ready' or 'active'; S2 never writes 'active' and never changes an existing row |

**Mechanism count: 89 core + 6 addendum (60-65).** Nothing in this document was removed, renumbered
or narrowed by S2: no existing gate, watchdog, recovery, suppression, lint or rewrite is touched, and
the delivery path's own supply mechanisms (17, 26, 28, 30, 32, 60, 61, 66, 79, 80) are unmodified.
