...
class LilyIdentityMixin:
    ...
    def on_dispatch_suppressed(self, act, speech_id, reason=None) -> None:
        """SEAM STUB for W1's hook of the same name (W1 owns the real hook
        in lily_speech_delivery; if its version lands in the MRO behind
        this mixin, the chain below reaches it). Recognition consumes the
        suppression, then defers to the next implementation."""
        self.note_recognition_dispatch_suppressed(act, speech_id, reason)
        nxt = getattr(super(), "on_dispatch_suppressed", None)
        if callable(nxt):
            nxt(act, speech_id, reason)
